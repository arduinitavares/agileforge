...
CONTAINER_FULL_GATE_TIMEOUT_MINUTES: int = 120
...
def test_workflow_has_required_runtimes(workflow: dict[str, object]) -> None:
    ...
    expected = {
        ...
        "linux-container": ("ubuntu-latest", CANONICAL_PYTHON),
    }
    assert set(_mapping(workflow["jobs"])) == {*expected, "frontend"}
...
def test_python_jobs_install_canonical_python_before_use(
    ...
    first_consumers = {
        ...
        "linux-container": "Build pinned Linux test image",
    }
    ...
    for job_name, consumer_name in first_consumers.items():
        ...
        assert setup_index < consumer_index
        if job_name == "python-313":
            assert len(install_indices) == 1
            assert setup_index < install_indices[0] < consumer_index
        else:
            assert str(steps[consumer_index]["run"]).startswith(
...
def test_linux_container_smoke_delegates_to_exact_repository_command(
    ...
    commands = [
        " ".join(run.split())
        for step in _steps(_job(workflow, "linux-container"))
        if isinstance((run := step.get("run")), str)
        and "scripts/ci_launcher_smoke.py" in run
    ]
    expected = (
        f"uv run --no-project --python {CANONICAL_PYTHON} python "
        "scripts/container.py --project-name agileforge-ci exec -- "
        "uv run --locked python scripts/ci_launcher_smoke.py "
        "--profile ci-linux-${{ github.run_id }}-${{ github.run_attempt }} "
...
def test_linux_container_prepares_workspace_before_smoke_and_full_gate(
    workflow: dict[str, object],
) -> None:
    """Run the real smoke and unchanged full gate on the imported Linux source."""
    job = _job(workflow, "linux-container")
    assert job["timeout-minutes"] == CONTAINER_FULL_GATE_TIMEOUT_MINUTES
    steps = _steps(job)
    names = [step.get("name") for step in steps]
    required_order = [
        "Build pinned Linux test image",
        "Copy the exact committed source into a named Linux workspace",
        "Exercise launcher lifecycle in the Linux container",
        "Run the canonical gate through the host transport",
    ]
    indices = [names.index(name) for name in required_order]
    assert indices == sorted(indices)
    controller = (
        f"uv run --no-project --python {CANONICAL_PYTHON} python scripts/container.py"
    )
    assert steps[indices[0]]["run"] == (
        f"{controller} build --target test --tag agileforge-test:ci"
    )
    source = " ".join(str(steps[indices[1]]["run"]).split())
    for required in (
        "docker volume create agileforge-ci-workspace",
        "docker volume create agileforge-ci-cache",
        "--name agileforge-ci-dev --platform linux/amd64",
        "--mount type=volume,src=agileforge-ci-workspace,dst=/workspace",
        "--mount type=volume,src=agileforge-ci-cache,dst=/cache",
        "agileforge-test:ci",
        f"{controller} --project-name agileforge-ci import-source",
        "docker exec -w /workspace/repos/agileforge agileforge-ci-dev uv sync --locked",
    ):
        assert required in source
    assert "type=bind" not in source
    assert steps[indices[3]]["run"] == (
        f"{controller} --project-name agileforge-ci exec -- ./agileforge-dev check"
    )


def test_linux_container_rehearses_production_and_compose_after_full_gate(
    workflow: dict[str, object],
) -> None:
    """Keep installed-image and persistent-state checks behind the canonical gate."""
    steps = _steps(_job(workflow, "linux-container"))
    names = [step.get("name") for step in steps]
    gate_index = names.index("Run the canonical gate through the host transport")
    production_index = names.index("Build and rehearse the installed production image")
    compose_index = names.index("Verify normal Compose startup and persistent state")
    assert gate_index < production_index < compose_index
    runner = f"uv run --no-project --python {CANONICAL_PYTHON} python"
    assert str(steps[production_index]["run"]).splitlines() == [
        f"{runner} scripts/container.py build --target production "
        "--tag agileforge-production:ci",
        f"{runner} scripts/verify_container.py --image agileforge-production:ci "
        '--expect-sha "$GITHUB_SHA"',
    ]
    assert steps[compose_index]["run"] == (
        f"{runner} scripts/verify_compose.py --image agileforge-production:ci "
        '--expect-sha "$GITHUB_SHA"'
    )
    for index in (gate_index, production_index, compose_index):
