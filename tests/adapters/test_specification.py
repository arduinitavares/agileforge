...
from adapters.adk.agents.specification import root_agent
...
def test_compiler_instructions_require_accepted_must_coverage() -> None:
    ...
    assert "built-in framework routing and Flatiron Director fallback" in (instructions)
...
def test_compiler_instructions_document_host_semantic_ids() -> None:
    ...
    assert (
        "semantic-only IDs from invariant type and canonical parameters" in instructions
    )
...
class TestInvariantClassificationRules:
    ...
    def test_max_value_mapping(self) -> None:
        ...
        assert invariant.parameters.max_value == EXPECTED_MAX_VALUE
...
class TestSpecAuthorityCompilerAgentIntegration:
    ...
    Skip with: pytest -m "not integration"
        tests/test_spec_authority_compiler_agent.py -v
    """
