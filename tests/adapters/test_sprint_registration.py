...
class TestSprintPlannerToolRegistration:
    ...
    STATES_WITH_SPRINT_PLANNER: ClassVar[list[OrchestratorState]] = [
            ...
            if name == "sprint_planner_tool":
                if isinstance(tool, BaseTool):
                    ...
                    assert callable(tool), (
                        "sprint_planner_tool should be callable or BaseTool, got "
                        f"{type(tool)}"
                    )
                ...
                return
        pytest.fail("sprint_planner_tool not found in SETUP_REQUIRED tools")
    ...
    def test_all_states_with_sprint_planner(self) -> None:
        """Verify the planner is registered in every expected state."""
