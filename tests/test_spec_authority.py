...
class TestSpecRegistryVersioning:
    ...
    def test_register_spec_version_creates_draft(
        self, session: Session, sample_project: Project, sample_spec_content: str
    ) -> None:
        ...
        assert spec.project_id == sample_project.project_id
    ...
    def test_register_spec_version_computes_correct_hash(
        ...
        result_v1 = register_spec_version(
            {"project_id": sample_project.project_id, "content": content_v1},
            tool_context=None,
        )
    ...
    def test_register_multiple_versions_for_same_project(
        self, session: Session, sample_project: Project
    ) -> None:
        """Test creating multiple spec versions for the same project."""
        v1_result = register_spec_version(
            {"project_id": sample_project.project_id, "content": "Version 1"},
            tool_context=None,
        )
        v2_result = register_spec_version(
            {"project_id": sample_project.project_id, "content": "Version 2 (changed)"},
            tool_context=None,
        )
...
class TestExplicitCompilation:
    ...
    def test_compile_stores_compiler_version_and_prompt_hash(
        ...
        assert authority is not None
        assert authority.compiler_version == "3.0.0"  # Expected constant
...
class TestAuthorityStatusCheck:
    ...
    def test_status_tool_delegates_to_compiler_service(
        self, sample_project: Project, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """Tool entrypoint should remain a thin compatibility adapter."""
        expected = {
            "success": True,
        ...
        assert result is expected
        assert captured["params"] == {"project_id": sample_project.project_id}
    ...
    def test_status_not_compiled_when_no_spec_exists(
        ...
        result = check_spec_authority_status(
            {"project_id": sample_project.project_id}, tool_context=None
        )
    ...
    def test_status_stale_when_new_approved_spec_after_compilation(
        self, session: Session, sample_project: Project, compiler_stub: object
    ) -> None:
        """Test status transitions to STALE when spec changes after compilation."""
        # Version 1: register, approve, compile
        del session, compiler_stub
        v1_result = register_spec_version(
            {"project_id": sample_project.project_id, "content": "Version 1 content"},
            tool_context=None,
        )
...
class TestDeterministicRetrieval:
    ...
    def test_get_compiled_authority_fails_for_wrong_project(
        self,
        session: Session,
        sample_project: Project,
        sample_spec_content: str,
        ...
        other_project = Project(name="Other Project", description="Test")
