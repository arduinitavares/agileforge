...
def test_authority_curate_v2_repairs_typed_invariant_parameter_capability(
    engine: Engine,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Typed forbidden-capability repairs must bind to parameters.capability."""
    ensure_schema_current(engine)
    fixture = _insert_rejected_authority_with_feedback(engine)
    invariant_id = "INV-forbidden-capability"
    replacement_capability = (
        "remote MLflow tracking server, model registry, deployment, "
        "serving, or production monitoring"
    )
    with Session(engine) as session:
        authority = session.get(CompiledSpecAuthority, fixture.authority_id)
        assert authority is not None
        ...
        compiled["invariants"].append(
            {
                ...
                "source_item_id": "CONSTRAINT.mlflow-local-only",
                "source_level": "MUST_NOT",
                "parameters": {
                    "capability": (
                        "adding MLflow or another experiment-tracking framework"
                    ),
                },
            }
        )
        ...
        feedback.feedback_json = json.dumps(
            {
                "feedback_items": [
                    {
                        "feedback_id": "AFB-parameter-capability",
                        "target_kind": "invariant",
                        "target_id": invariant_id,
                        "issue_type": "materially_wrong_invariant",
                        "severity": "blocking",
                        "instruction": (
                            "Repair this capability. Local MLflow tracking is "
                            "approved; only remote/server/registry/deployment "
                ...
                "repairs": [
                    {
                        ...
                        "target_handle": "R1",
                        "repair_kind": "replace_parameter_text",
                        "replacement_text": replacement_capability,
                    }
                ]
            },
            "quality_report": {"status": "passed"},
    ...
    result = runner.curate(
        AuthorityCurationRequest(
            ...
            expected_source_authority_fingerprint=authority_fingerprint,
            feedback_attempt_id=fixture.feedback_attempt_id,
            idempotency_key="curate-v2-parameter-capability",
        )
    )
    ...
    inputs = cast("curation_mod._CurationWorkflowInputs", captured["inputs"])
    repair_menu = cast("list[dict[str, object]]", inputs.repair_menu)
    assert repair_menu[0]["target_field"] == "parameters.capability"
    assert repair_menu[0]["allowed_repair_kinds"] == [
        "replace_parameter_text",
        "mark_unresolvable",
    ]
    ...
    repaired = [
        item
        for item in candidate["invariants"]
        if item.get("source_item_id") == "CONSTRAINT.mlflow-local-only"
    ]
    assert len(repaired) == 1
    assert repaired[0]["id"] != invariant_id
    assert repaired[0]["parameters"]["capability"] == replacement_capability
...
def test_authority_curate_v2_validates_selection_payload_when_gate_fails(
    engine: Engine,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Gate failure must not hide malformed v2 repair selections."""
    ensure_schema_current(engine)
    fixture = _insert_rejected_authority_with_feedback(engine)
    fake_workflow = FakeWorkflowPort()
    fake_workflow.update_session_status(
    ...
    result = runner.curate(
        AuthorityCurationRequest(
            ...
            feedback_attempt_id=fixture.feedback_attempt_id,
            compiler_model="test-curation-model",
            idempotency_key="curate-v2-validates-selection-before-gate",
        )
    )
...
def test_authority_curate_v2_rejects_valid_selection_when_gate_keeps_feedback_open(
    ...
    """A valid handle selection must not bypass target-feedback gate failure."""
    ensure_schema_current(engine)
    fixture = _insert_rejected_authority_with_feedback(engine)
    fake_workflow = FakeWorkflowPort()
    fake_workflow.update_session_status(
    ...
    result = runner.curate(
        AuthorityCurationRequest(
            ...
            feedback_attempt_id=fixture.feedback_attempt_id,
            compiler_model="test-curation-model",
            idempotency_key="curate-v2-valid-selection-gate-fail",
        )
    )
