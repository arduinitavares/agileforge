...
def build_authority_curation_workflow(*, model: str | BaseLlm) -> Workflow:
    ...
    repair_compiler = LlmAgent(
        ...
        instruction=(
            ...
            "JSON with targeted patch operations in patches. Do not copy or "
            "rewrite the full authority JSON for targeted repairs. Preserve "
            "untouched invariants, ids, source references, and ordering by "
            "omitting them from patches. Never use authority:* ids as patch "
            "targets; when broad authority-candidate feedback names concrete "
            "ASM-*, GAP-*, or INV-* ids in its instruction, patch those "
            "concrete assumption, gap, or invariant ids only. Mark "
            "unresolved feedback ids and "
            "failure_reason "
