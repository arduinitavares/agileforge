...
| --- | --- | --- |
| **You are here** | `GET /api/projects/{project_id}/position` and its `data.decisions` | It is the set of eligible required/recovery graph decisions, never a percentage, a stage number, an action-list order, or the first row in a list. Several stages may be marked **Here**. Optional re-entry is shown separately as optional. |
...
The Project lifecycle says a stage can contain multiple commands and review decisions. Therefore the map uses this derivation, aligned with the immediate-work filtering in `cli/workflow_commands.py:391`:
...
2. Build `currentStageIds` from decisions whose recommendation is `required` or `recovery` and whose category is `available`; add category `waiting` only for the supported review decision kinds: `decide_backlog`, `decide_product_goal_review`, `decide_roadmap`, `decide_sprint_plan`, `decide_specification`, `decide_story`, `decide_vision_review`, and `review_sprint`. This is a presentation filter for current required work. It does not depend on `compareLifecycleDecisions()`, a frontend action priority, or an optional action being transportable.
3. Exclude every `optional_reentry` decision from `currentStageIds`, including `retry_sprint` and correction routes; render it in an explicit **Optional re-entry** section on its associated card. Render a map card as **Here** when its stage ID occurs in the nonempty set. Multiple legitimate cards may be **Here**. If the set is empty, show **No required step available** and the returned blocked/waiting reason(s); do not default the current marker to Stories.
4. Render **Viewing** from local selection. On first entry only, the browser may select the lowest numbered `currentStageIds` item as a navigation default; that choice is not a current-stage assertion and must not replace an existing selection. `resolveActiveWorkflowStage()`, `compareLifecycleDecisions()` and `updateStageView()` are not sufficient as-is because they choose one old group and apply that style as the only current state.
