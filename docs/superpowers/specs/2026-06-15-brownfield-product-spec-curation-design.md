...
| FR-003 | Brownfield raw inputs use source terminology, not spec terminology. | Brownfield create or scan may accept `--source-file`, `--repo-path`, and user notes; brownfield mode rejects `--spec-file` as raw input. | Must |
...
agileforge brownfield scan --project-id 42 --repo-path /workspace/invoice-portal --source-file notes.md
...
| --- | --- | --- | --- |
| `SETUP_REQUIRED` | `brownfield_curation_required` | Brownfield source/scan/draft/approval is still in progress; no compileable spec version is installed. | `brownfield scan`, `brownfield spec draft`, `brownfield spec approve` when derived guards allow it |
...
- `project_id`
- `attempt_id` for attempt-like artifacts
...
1. registers the curated spec in `SpecRegistry`
2. records approval metadata with draft and chain fingerprints
3. writes `setup_spec_file_path`, `setup_spec_hash`, and `setup_spec_version_id`
4. transitions `setup_status` to `authority_compile_required`
5. writes `setup_next_actions=[authority compile action]`
6. persists the idempotency response

If any side effect fails after the mutation starts, recovery must be explicit
through the mutation ledger. A replay must either return the original response
or report a stale/idempotency conflict.

### 6.7 Error And Warning Codes
...
### 6.8 Implementation-Heavy Draft Detection
...
- The storage backing for brownfield artifacts may be workflow state, database
  tables, or another existing artifact store, but the external contracts in this
  document must remain stable.
...
| --- | --- | --- | --- |
| Should the first implementation store brownfield artifacts in workflow state or a dedicated table? | Affects migration shape and query ergonomics, but not the public contracts. | Implementation agent | Deferred to implementation plan |
| Should human-edited curated specs be imported through `brownfield spec draft` or a separate `brownfield spec import` command? | Affects CLI ergonomics for manual review edits, but approval still uses explicit draft `attempt_id`. | Implementation agent | Deferred to implementation plan |
